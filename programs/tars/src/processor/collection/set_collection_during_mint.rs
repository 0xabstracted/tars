...
use crate::{cmp_pubkeys, Tars, CollectionPDA};

/// Sets and verifies the collection during a tars mint
#[derive(Accounts)]
pub struct SetCollectionDuringMint<'info> {
    ...
    payer: Signer<'info>,
    #[account(mut, seeds = [b"collection".as_ref(), tars.to_account_info().key.as_ref()], bump)]
...
pub fn handle_set_collection_during_mint(ctx: Context<SetCollectionDuringMint>) -> Result<()> {
    ...
    let tars_key = ctx.accounts.tars.key();
    ...
    if !cmp_pubkeys(&mint_ix_cm, &tars_key) {
        msg!(
            "Tars with pubkey {} does not match the mint ix Tars with pubkey {}",
            mint_ix_cm,
            tars_key
        );
        return Ok(());
    ...
    let seeds = [b"collection".as_ref(), tars_key.as_ref()];
    let bump = assert_derivation(&crate::id(), &collection_pda.to_account_info(), &seeds)?;
    let signer_seeds = [b"collection".as_ref(), tars_key.as_ref(), &[bump]];
