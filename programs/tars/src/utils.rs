...
use crate::{TarsError, Tars};

pub fn assert_initialized<T: Pack + IsInitialized>(account_info: &AccountInfo) -> Result<T> {
    let account: T = T::unpack_unchecked(&account_info.data.borrow())?;
    if !account.is_initialized() {
        Err(TarsError::Uninitialized.into())
    } else {
        Ok(account)
    }
}
...
pub fn assert_valid_go_live<'info>(
    ...
    match tars.data.go_live_date {
        None => {
            if !cmp_pubkeys(payer.key, &tars.authority) {
                return Err(TarsError::TarsNotLive.into());
            }
        }
        Some(val) => {
            if clock.unix_timestamp < val && !cmp_pubkeys(payer.key, &tars.authority) {
                return Err(TarsError::TarsNotLive.into());
            }
        }
    }
...
pub fn assert_owned_by(account: &AccountInfo, owner: &Pubkey) -> Result<()> {
    if !cmp_pubkeys(account.owner, owner) {
        Err(TarsError::IncorrectOwner.into())
    } else {
        Ok(())
    }
}
...
pub fn spl_token_transfer(params: TokenTransferParams<'_, '_>) -> Result<()> {
    ...
    result.map_err(|_| TarsError::TokenTransferFailed.into())
}
...
pub fn assert_keys_equal(key1: &Pubkey, key2: &Pubkey) -> Result<()> {
    if !cmp_pubkeys(key1, key2) {
        err!(TarsError::PublicKeyMismatch)
    } else {
        Ok(())
    }
}
...
pub fn spl_token_burn(params: TokenBurnParams<'_, '_>) -> Result<()> {
    ...
    result.map_err(|_| TarsError::TokenBurnFailed.into())
}
...
pub fn punish_bots<'a>(
    ...
    msg!(
        "{}, Tars Botting is taxed at {:?} lamports",
        error.to_string(),
        fee
    );
